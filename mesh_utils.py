from GalaDyn.satellites_utils import Satellite
import numpy as np
import pandas as pd
import gc
...
class mesh:
    ...
    #     print("saving data")
        if mode_stars is not None:
            mesh_completa.to_csv(path_acceleration + f"mesh_aceleracion_{comp}_{mode_stars}_{self.name}_ytRS.csv", sep = ",")
        else:
            mesh_completa.to_csv(path_acceleration + f"mesh_aceleracion_{comp}_{self.name}_ytRS.csv" , sep = ",")
    ...
    def plot_acceleration_components (self, comp, rango_z=1e-14, rango_r=1e-13, rango_phi=1e-12, mode_stars=None):
        ...
        if mode_stars is not None:
            plt.savefig(path_figures + f"acceleration_{comp}_{mode_stars}_{self.name}_ytRS.png", format='png', dpi=150, bbox_inches='tight')
        else:
            plt.savefig(path_figures + f"acceleration_{comp}_{self.name}_ytRS.png", format='png', dpi=150, bbox_inches='tight')
    ...
    def satellites_acceleration_id (self, sat, plot = True, rango=1e-17):
            ...
            #TODO: add case with all satellites
        #---------------------PROGENITOR------------------------------------
            mesh_core = self.calculate_force(DM_core_i)
            #az_mean_prog = np.mean(np.abs(az_core))

        #---------------------STREAMS------------------------------

            mesh_stream= self.calculate_force(DM_stream_i)
