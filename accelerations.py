...
def acceleration_in_mesh_comp (name,mode,mode_stars, path_disk, plot = True, tidal = False):
    ...
    if mode_stars is not None:
      ...
      mesh.to_csv(path_acceleration + "mesh_aceleracion_%s_%s_ytRS.csv" %(mode,name), sep = ",")
...
def plot_acceleration_components (name, mode, mesh_x, mesh_y, az, ar,aphi, rango_z=1e-14, rango_r=1e-13, rango_phi=1e-13, mode_stars=None):
    ...
    if mode_stars is not None:
        plt.savefig(path_figures + "acceleration_%s_%s_%s_ytRS.png"%(mode,mode_stars, name), format='png', dpi=150, bbox_inches='tight')
    else:
        plt.savefig(path_figures +"acceleration_%s_%s_ytRS.png"%(mode,name), format='png', dpi=150, bbox_inches='tight')
    
    gc.collect()

#-----------------------FOR SATELLITES AS POINTS -----------------------------------------

def acceleration_satellite_as_point (x_bin, y_bin, z_bin, sat, mesh_phi,satelite, name, tidal = True):

    G = 1.3273e11
    kpc_to_km = 3.086e16
    sat = pd.read_csv(path_datos + "satelite_%s_coordinates.csv"%satelite ,
                               sep = ",")
    sat_mass = pd.read_csv(path_datos + "%s_complete_mass_table.csv"%satelite ,
                               sep = ",")
...
def acceleration_satellites_all_plot (name, nbins = 75 ):
  #  lookback = datos_edades.loc[datos_edades['Snapshot'] == name, 'Lookback'].iloc[0]
    ...
    plt.savefig(path_figures + "acceleration_satelites_%s_ytRS_as_point.png"%name, format='png', dpi=150, bbox_inches='tight')
...
def separate_stream_remanent (sat, name, coord, mass):
   ...
   # masa = mass.loc[mass['Snapshot'] == name, 'Mass'].iloc[0]
    ...
    DM= pd.read_csv(path_crossmatch +"%s_%s_crossmatch_dm.csv" %(sat,name), sep = ",")
    stars= pd.read_csv(path_crossmatch +"%s_%s_crossmatch_stars.csv" %(sat,name), sep = ",")
